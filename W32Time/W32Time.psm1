...
PROCESS {
    ...
    if ( $__boundParameters["Verbose"]) {
         Write-Verbose -Verbose -Message C:\Windows\System32\w32tm.exe
    ...
    if ( $PSCmdlet.ShouldProcess("C:\Windows\System32\w32tm.exe $__commandArgs")) {
    # check for the application and throw if it cannot be found
        if ( -not (Get-Command -ErrorAction Ignore "C:\Windows\System32\w32tm.exe")) {
          throw "Cannot find executable 'C:\Windows\System32\w32tm.exe'"
        }
        if ( $__handlerInfo.StreamOutput ) {
            & "C:\Windows\System32\w32tm.exe" $__commandArgs | & $__handler
        }
        else {
            $result = & "C:\Windows\System32\w32tm.exe" $__commandArgs
            & $__handler $result
        }
    }
  } # end PROCESS
...
.DESCRIPTION See help for C:\Windows\System32\w32tm.exe
