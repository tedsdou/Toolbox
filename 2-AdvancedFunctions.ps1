...
[System.Net.Dns]::GetHostByAddress('192.168.10.10')
...
Resolve-Reverse -IP '192.168.10.20'
#EndRegion
...
#Region DNS change example
Function Set-DNSServerSettings {
    [CmdletBinding(SupportsShouldProcess = $true,
        ConfirmImpact = 'Medium')]
    Param([string]$server)
    $strDNSServers = '192.168.1.1', '192.168.1.2'
    $NetItems = @(Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $server | Where-Object { ($_.IPEnabled -eq $true) -and ($_.DNSServerSearchOrder -ne $null) } )
        
    foreach ($objItem in $NetItems) {
        #Write-Host "The current DNS Search Order is: $($objItem.DNSServerSearchOrder)" 
        if ($pscmdlet.ShouldProcess($server, "Changing DNSServerSearchOrder on $($objItem.Description)")) {       
            Try {
                $ErrorActionPreference = 'Stop'
                $objItem.SetDNSServerSearchOrder($strDNSServers)
            }
            Catch {
                Write-Warning -Message "Houston, we have a problem: $($_.Exception.Message), for $objItem on $server"       
            }
            Finally {
...
Set-DNSServerSettings -server WIN10 -WhatIf
#endregion DNS
#EndRegion Whatif
...
Get-Something -Data (Get-ChildItem C:\Temp) -First 10

Get-Something -Data (Get-ChildItem C:\Temp) -Skip 10
