...
[CmdletBinding()]
param(
    [Parameter(HelpMessage = 'Channel to download, Valid Options are: Dev, Beta, Stable, EdgeUpdate, Policy')]
    [ValidateSet('Dev', 'Beta', 'Stable', 'EdgeUpdate', 'Policy')]
    [string]$Channel = 'Stable',
  
    [Parameter(HelpMessage = 'Folder where the file will be downloaded')]
    [ValidateNotNullOrEmpty()]
    [string]$Folder = 'C:\Temp',

    [Parameter(HelpMessage = 'Platform to download, Valid Options are: Windows or MacOS')]
    [ValidateSet('Windows', 'MacOS', 'any')]
    [string]$Platform = 'Windows',

    [Parameter(HelpMessage = 'Architecture to download, Valid Options are: x86, x64, arm64, any')]
    [ValidateSet('x86', 'x64', 'arm64', 'any')]
    [string]$Architecture = 'x64',

    [parameter(HelpMessage = 'Specifies which version to download')]
    [ValidateNotNullOrEmpty()]
    [string]$ProductVersion = '98.0.1108.55',

    [parameter(HelpMessage = 'Overwrites the file without asking')]
    [Switch]$Force
)
        ...
        if (Test-Path "$Folder\$fileName" -ErrorAction SilentlyContinue) {
            if ($Force) {
                ...
                try {
                    ...
                    throw "Attempted to download file, but failed: $($_.Exception.Message)"
                }    
            }
            else {
